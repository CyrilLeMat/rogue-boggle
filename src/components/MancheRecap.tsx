...
export function MancheRecap() {
  ...
  const modifiers = result.euros - (result.eurosBase + result.eurosBonus + result.eurosTime + result.eurosQuest);
  ...
  return (
    <div className="panel recap">
      ...
      <div className="euros-breakdown">
        ...
        <div className="euro-line total"><span>Gagné</span><span className="euros">{result.eurosBase + bonusShown + result.eurosTime + result.eurosQuest + (done ? modifiers : 0)} €</span></div>
      </div>

      <p className="muted">{result.words.length} mots</p>
      <div className="cols">
        <div>
          <h3>Meilleurs mots</h3>
          <ul>{best.map((w) => <li key={w.word}>{w.word} <span className="pts">{w.score}</span></li>)}</ul>
        </div>
        <div>
          <h3>Manqués</h3>
          <ul>{result.missed.map((w) => <li key={w}>{w}</li>)}</ul>
        </div>
      </div>
