...
function relicCandidates(input: ShopInput): Candidate[] {
  return RELICS
    ...
    .map((r) => ({
      item: { kind: 'relic' as const, id: r.id, price: RARITY_PRICE[r.rarity], sold: false },
...
// 3 emplacements : 2 tirés parmi relics + consommables, 1 parmi malédictions + consommables.
// `keep` : articles déjà achetés à conserver à leur place lors d'un changement.
