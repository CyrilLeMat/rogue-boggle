...
export interface EpilogueBeat { text: string; kind?: 'quote' | 'fall' }
...
function lastDay(i: EpilogueInput): EpilogueBeat[] {
  return i.victory
    ? [{ text: 'Le dernier jour de CE2 tombe un mardi. Il fait vingt-huit degrés dans la classe et plus personne ne fait semblant de travailler.' }]
    : [
      { text: `Tu as crié « {cri} » en voyant la note de {maitresse}. La dictée ${i.manche} a eu raison de toi, et tes parents sont convoqués.` },
      { text: 'Le dernier jour de CE2 tombe un mardi. Tu le passeras deux fois.' },
    ];
}
...
function teacher(i: EpilogueInput): EpilogueBeat[] {
  if (!i.victory) {
    return [
      { text: '{Maitresse} remplit la case « appréciation générale » sans lever le stylo. Elle a fini avant toi.' },
      { text: 'Une année, ce n\'est rien. Je t\'attends en septembre, à la même place, et cette fois nous irons jusqu\'au bout. J\'ai tout mon temps, {nom}.', kind: 'quote' },
    ];
  }
  if (i.moyenne >= 16) {
    return [
      { text: '{Maitresse} écrit trois lignes serrées dans la case « appréciation générale », puis une flèche, puis elle continue au dos.' },
      { text: 'Je n\'ai rien à ajouter. J\'ajoute quand même : ne change pas, et n\'oublie pas qui t\'a appris à écrire.', kind: 'quote' },
    ];
  }
  if (i.moyenne >= 12) {
    return [
      { text: '{Maitresse} relit ta copie une dernière fois, à la lumière de la fenêtre, comme on vérifie un billet de banque.' },
      { text: 'Année honorable. Peut mieux faire, mais tout le monde peut mieux faire, moi la première.', kind: 'quote' },
    ];
  }
  return [
    { text: '{Maitresse} hésite longtemps devant la case « appréciation générale », puis écrit une phrase, la rature, et en écrit une autre.' },
    { text: 'Passe. Passe, et va vivre ta vie loin de cette salle. Ce n\'est pas un reproche. Enfin, pas seulement.', kind: 'quote' },
  ];
}
...
function kevin(i: EpilogueInput): EpilogueBeat {
  if (!i.duelDone) {
    return { text: 'Kévin, lui, passe en CM1. Il n\'a pas eu à se forcer : il a copié sur quelqu\'un d\'autre, et ce quelqu\'un d\'autre n\'a rien dit.' };
  }
  if (i.duelWon) {
    return { text: 'Kévin n\'a plus jamais copié sur personne. On raconte dans la cour qu\'il s\'est mis à réviser le soir. On raconte n\'importe quoi dans cette cour, mais cette fois-là c\'était vrai.' };
  }
  return i.stolen
    ? { text: `Kévin passe en CM1 avec « ${i.stolen} » au fond de son cartable. Il s'en sert encore, mal, devant des élèves qui ne savent pas d'où ça vient.` }
    : { text: 'Kévin passe en CM1 sans t\'adresser un regard. Il a gagné quelque chose cette année, et vous êtes deux à savoir quoi.' };
}
...
function numbers(i: EpilogueInput): EpilogueBeat {
  if (!i.bestWord) return { text: `Tu as rendu ${i.manche} copies cette année. Tu ne te souviens d'aucune.` };
  return {
